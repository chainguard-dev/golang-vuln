...
func (h *TextHandler) vulnerability(index int, findings []*findingSummary) {
	...
	for _, module := range byModule {
		...
		if stdPkg := h.pkg(module); path == pkg.GoStdModulePath && stdPkg != "" {
			path = stdPkg
		}
...
func isStdFindings(findings []*findingSummary) bool {
	for _, f := range findings {
		if vulncheck.IsStdPackage(f.Trace[0].Package) || f.Trace[0].Module == pkg.GoStdModulePath {
			return true
		}
	}
	return false
