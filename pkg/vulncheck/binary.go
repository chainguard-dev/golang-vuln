...
// BinaryEmitResults detects presence of vulnerable symbols in bin and emits
// findings to handler.
func BinaryEmitResults(ctx context.Context, handler govulncheck.Handler, bin *Bin, cfg *govulncheck.Config, client *client.Client) error {
...
func Binary(ctx context.Context, handler govulncheck.Handler, bin *Bin, cfg *govulncheck.Config, client *client.Client) (*Result, error) {
	...
	mods := append(bin.Modules, graph.GetModule(pkg.GoStdModulePath))
...
func binImportedVulnPackages(graph *PackageGraph, pkgSymbols map[string][]string, affVulns affectingVulns) []*Vuln {
	var vulns []*Vuln
	for pkgSym := range pkgSymbols {
		for _, osv := range affVulns.ForPackage(pkg.UnknownModulePath, pkgSym) {
			vuln := &Vuln{
				OSV:     osv,
				Package: graph.GetPackage(pkgSym),
			}
...
func binVulnSymbols(graph *PackageGraph, pkgSymbols map[string][]string, affVulns affectingVulns) []*Vuln {
	var vulns []*Vuln
	for p, symbols := range pkgSymbols {
		for _, symbol := range symbols {
			for _, osv := range affVulns.ForSymbol(pkg.UnknownModulePath, p, symbol) {
				vuln := &Vuln{
					OSV:     osv,
					Symbol:  symbol,
					Package: graph.GetPackage(p),
				}
				vulns = append(vulns, vuln)
			}
		}
