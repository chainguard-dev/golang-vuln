...
// Otherwise, returns pkg.UnknownPackagePath.
func pkgPath(f *ssa.Function) string {
	...
	return pkg.UnknownPackagePath
}

func pkgModPath(p *packages.Package) string {
	if p != nil && p.Module != nil {
		return p.Module.Path
	}
	return pkg.UnknownModulePath
}

func IsStdPackage(p string) bool {
	if p == "" || p == pkg.UnknownPackagePath {
		return false
	}
